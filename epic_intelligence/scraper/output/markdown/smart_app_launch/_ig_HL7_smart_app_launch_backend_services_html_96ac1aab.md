...
> Source: https://build.fhir.org/ig/HL7/smart-app-launch/backend-services.html
...
- Backend ServiceBackend ServiceFHIR authorization serverFHIR authorization serverFHIR resource serverFHIR resource server
opt[Precondition: Client Registration](may be out of band)Discovery requestDiscovery responseAccess token requestalt[Granted]Access token responseRequest Resources[Denied]Authorization error

 [Register Backend Service](#step-1-register) (*one-time step*, can be out-of-band)

 - [Retrieve .well-known/smart-configuration](#step-2-discovery)

 - [Obtain access token](#step-3-access-token)

 - [Access FHIR API](#step-4-fhir-api)
...
the following parameters:
 ...
 `scope` | 
 required | 
 The scope of access requested. See note about scopes below | 
 |
...
the following properties:
 ...
 `expires_in` | 
 required | 
 The lifetime in seconds of the access token. The recommended value is `300`, for a five-minute token lifetime. | 
 |
