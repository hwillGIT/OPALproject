...
> Source: https://build.fhir.org/ig/HL7/smart-app-launch/app-state.html
...
## Persisting App State (Experimental)
 ...
 - [`smart-app-state` capability EXP](#smart-app-state-capability-exp)

 - [Formal definitions](#formal-definitions)

 - [Discovery](#discovery)

 - [App State Interactions](#app-state-interactions)

 - [Managing app state (CRUDS)](#managing-app-state-cruds)
...
### `smart-app-state` capability EXP
...
The EHR’s App State FHIR endpoint SHALL provide support for:
 ...
 - `PUT /Basic/[id]` requiring the presence of `If-Match` to prevent contention

 - `DELETE /Basic/[id]` requiring the presence of `If-Match` to prevent contention

 - `GET /Basic?code={}&subject={}`

 - `GET /Basic?code={}&subject:missing=true` // for global app config
...
The request body is a `Basic` resource where:
 ...
 - Total resource size as serialized in the POST body SHALL NOT exceed 256KB unless the EHR’s documentation establishes a higher limit

 - `Basic.id` SHALL NOT be included

 - `Basic.meta.versionId` SHALL NOT be included

 - `Basic.subject.reference` is optional, associating App State with [at most one subject](#at-most-one-subject). When omitted, global configuration can be stored in App State. When present, this SHALL be an absolute reference to a resource in the EHR’s primary FHIR server. The EHR SHALL support at least Patient, Practitioner, PractitionerRole, RelatedPerson, Person. The EHR’s documentation MAY establish support for a broader set of resources.

 - `Basic.code.coding[]` SHALL include exactly one app-specified Coding

 - `Basic.extension` Extensions SHALL be limited to the `valueString` type unless the EHR’s documentation establishes a broader set of allowed extension types
...
The payload is a `Basic` resource where:
 ...
 - `Basic.id` SHALL be present, matching the `[id]` in the request URL

 - `Basic.subject` and `Basic.code` SHALL NOT change from the previously-persisted values
